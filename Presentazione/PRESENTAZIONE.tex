...
\institute{\large{Università degli Studi di \\Trento}}
...
Sorgente $S$ distribuita come una variabile casuale di Bernoulli di parametro $\epsilon$
\begin{columns}[onlytextwidth]
\column{0.4\textwidth}
...
%\vspace{20pt}
\begin{block}{ }
\large\textbf{Capacità canale binario simmetrico:}\\  $$C=1-H_b(p)$$
\end{block}
\end{columns}
\end{frame}

\begin{frame}[t]{Velocità} \vspace{0.1pt}
\begin{block}{Velocità di Trasmissione}
Viene detta \textbf{velocità di trasmissione} il numero di bits d'informazione che passano attraverso un canale
\end{block}
\begin{columns}[onlytextwidth]
\column{0.5\textwidth}
\only<1>{
...
\vspace{20pt}
\\
Nel teorema di Shannon verranno codificate parole di lunghezza $dV$ con parole di lunghezza d.\\
Il numero totale delle parole da codificare  sarà $M=2^{dV}$ e le parole codice corrispondenti verranno scelte con distribuzione uniforme.
\vspace{10pt}	
}
\end{frame}
...
\center{\large\textbf{{GRAZIE DELL'ATTENZIONE}}}
\end{frame}
